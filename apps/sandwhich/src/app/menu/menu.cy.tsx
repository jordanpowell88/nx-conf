...
describe(Menu.name, () => {
  it('mounts Menu', () => {
    cy.mount(<Menu peanutButterChange={cy.spy()} jellyChange={cy.spy()} extraChange={cy.spy()} />)
  ...
  it('handles menu item changes', () => {
    cy.mount(<Menu peanutButterChange={cy.spy().as('pbChange')} jellyChange={cy.spy().as('jellyChange')} extraChange={cy.spy().as('extraChange')} />)
